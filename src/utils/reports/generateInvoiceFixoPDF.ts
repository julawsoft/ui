...
const fillTemplate = (template: string, processoSelected: IProcesso, cliente: IClient, processo: IProcesso, total: number) => {
  ...
  return template
    ...
    .replace("{{ref}}",  processoSelected.ref)
    .replace("{{assunto}}",  processoSelected.assunto)
    .replace("{{n_processo_judicial}}",  processoSelected.n_processo_judicial ?? '-' )
    .replace("{{fase}}", processoSelected.fase )
...
export const generateInvoiceFixoPDF = async (processoSelected: IProcesso, cliente: IClient, processo: IProcesso, total: number) => {
  ...
  const opt:any = {
    margin:       [10, 10, 20, 10],
    filename:     `invoice_honorarios_fixo_.pdf`,
