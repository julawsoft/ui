...
const TYPE_REQUEST = {
  ...
  INCIDENTE: 'Incidente',
  OUTROS: 'Outros',
  JUSTIFICACAO_AUSENCIA: 'Justificação de Ausência',
}
...
export function transformHomeCardAniversario(
  ...
  return (
    ...
    data.map((item: any, index: number) => {
      return {
        icon: <Cake size={32} fill='#FFF' color='#F2C112' />,
        userName: item.name,
        date: item.birthday,
        bgColor: '#FFF',
        bgColorIcon: "#15171C"
...
const swicthIcon = (type: string) => {
  switch (type) {
    case TYPE_REQUEST.DOENCA:
      return <MdOutlineHealing size={36} />
    case TYPE_REQUEST.FERIAS:
      return <GiPalmTree size={36} />
    default:
      return <IoMegaphoneOutline  size={36} />
  }
}
...
const swicthStatusBorderBottomColor = (status: string) => {
  switch (status) {
