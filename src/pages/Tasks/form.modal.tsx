...
interface TasksAgendaFormProps {
  handleSaveOrUpdate: (data: TaskFormData) => void;
  handleClose: () => void;
  isClose: boolean;
...
const TasksAgendaForm: React.FC<TasksAgendaFormProps> = ({
  tasks,
  handleSaveOrUpdate,
  handleClose,
  isClose,
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<TaskFormData & { horaParaRealizacao: string }>({
  ...
  useEffect(() => {
    if (tasks) {
      ...
      reset({
        ...
        gestorId: tasks.gestor_id ?? null,
        colaboradorId: tasks.colaborador_id ?? 0,
...
export default TasksAgendaForm;
